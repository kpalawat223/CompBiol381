...
This code rarefies the data. As far as I understand it, this essentially standardizes the microbial taxa data to the lowest number of values across sampels. Also shown is the structure of the data.
```{r}
input_rar = single_rarefy(input, min(colSums(input$data_loaded)))
...
This code calculates diversity indecies for the data. If you use the function "diversity()" you can export the index values.
```{r}
plot_diversity(input_rar, 'Sample_type', 'shannon')
plot_diversity(input_rar, 'Sample_type', 'simpson')
```

```{r}
plot_diversity(input_rar, 'Sample_Farming', 'shannon')
plot_diversity(input_rar, 'Sample_Farming', 'simpson')
```
