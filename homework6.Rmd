...
```{r}
z <- read.table("hw6data.csv",header=TRUE,sep=",", stringsAsFactors=FALSE)
names(z) <- list("ID","myVar")
str(z)
z <- z[-49,] #outlier removal
str(z)
summary(z)
```
...
p1 <- ggplot(data=z, aes(x=myVar, y=..density..)) +
  geom_histogram(color="black",fill="mintcream",size=0.2) 
print(p1)
```
...
Gamma Parameters and Simulating Data
```{r}
gammaPars <- fitdistr(z$myVar,"gamma")
shapeML <- gammaPars$estimate["shape"]
rateML <- gammaPars$estimate["rate"]
gammaID <- seq(from=1, to=length(z$myVar), by=1)
gammaVar <- rgamma(length(gammaID), shape = shapeML, rate = rateML)
gammaData <- as.data.frame(gammaID)
gammaData <- cbind(gammaData, gammaVar)
```

Simulated Data: Histogram
```{r}
gamma1 <- ggplot(data=gammaData, aes(x=gammaVar, y=..density..)) +
  geom_histogram(color="black",fill="mintcream",size=0.2) 
print(gamma1)
```

Simulated Data: Probability Density Curve
```{r}
gamma1 <-  gamma1 +  geom_density(linetype="dotted",size=0.75)
print(gamma1)
```

Experimental Data: Probability Density Curve
```{r}
p1 <- ggplot(data=z, aes(x=myVar, y=..density..)) +
  geom_histogram(color="black",fill="mintcream",size=0.2)
p1 <-  p1 +  geom_density(linetype="dotted",size=0.75)
...
The mean of the simulated data is much more central than the mean of the experimental data. But besides that, it seems to be simulating the data accurately. It has also 
